...
namespace CatEngine
{
	...
	char* ReadBytes(const std::string& filepath, uint32_t* outSize)
	{
		...
		if (!stream)
			CE_API_CRITICAL("Could not open file - {0}", filepath);
	...
	void ScriptEngine::InitMono()
	{
		...
		s_Data->CoreAssembly = LoadCSharpAssembly("Resources/Scripts/Cat-ScriptCore.dll");
		...
		MonoClass* monoClass = mono_class_from_name(assemblyImage, "CatEngine", "Main");
		...
		MonoString* monoString = mono_string_new(s_Data->AppDomain, "AHHHHHHHHHHHHHHHHH!");
